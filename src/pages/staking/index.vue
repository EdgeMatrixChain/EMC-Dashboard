<template>
  <div class="w-screen flex flex-col bg-[rgb(0,2,27)] fixed left-0 h-screen top-0 overflow-y-scroll">
    ...
    <!-- 第一页 -->
    <Banner :earnings="earnings" :totalStaked="totalStaked" />

    <!-- 第二页 -->
    <div class="w-full relative z-10 flex flex-col pt-[70px] lg:pt-[90px] flex-shrink-0">
      <h2 class="text-[32px] leading-[32px] lg:text-[48px] font-bold lg:leading-[84px] text-[#fff] flex justify-center">
        Staking with EMC</h2>
      <div class="max-w-[1340px] mx-auto flex w-full flex-1 px-5 lg:mt-[50px] mt-[40px] flex-shrink-0">
        <div
          class="bg-[#1A1C34] lg:pt-[76px] pt-[33px] flex-col lg:pb-[68px] pb-[50px] rounded-[24px] border-[2px] border-[#fff]/20 flex w-full mb-[30px]">
          <div class="flex flex-col lg:pl-[80px] pl-[15px]">
            <div class="flex items-center">
              <p class="mr-[10px] text-[24px] lg:text-[28px] leading-[24px] lg:leading-[28px] text-[#fff]">Duration (Days)
              </p>
            ...
            <template v-if="isDev">
              <div class="flex items-center lg:mt-[20px]">
                <p class="mr-[10px] text-[24px] lg:text-[28px] lg:leading-[28px] leading-[28px] text-[#fff]">Start Time
                </p>
              </div>
            ...
            <!-- <div class="flex lg:mt-[34px] mt-[20px] items-center">
                            ...
                            <span class="flex flex-col ml-[12px]">
                                ...
                                <div @click="phaseDecrease" class="w-6 overflow-hidden inline-block cursor-pointer mt-2">
                                    <svg width="14" height="9" viewBox="0 0 14 9" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path d="M7 9L13.0622 3.76515e-07L0.937823 -1.74337e-06L7 9Z" fill="white" />
                                    </svg>
                                </div>
                            </span>
                        </div>
            ...
            <!-- <div class="flex flex-col w-full lg:mt-[56px] mt-[20px] relative">
                            <div class="absolute left-0 top-0">
                                <svg xmlns="http://www.w3.org/2000/svg" width="9" height="123" viewBox="0 0 9 123"
                                    fill="none">
                                    <line x1="4.5" y1="-2.18557e-08" x2="4.50001" y2="123"
                                        stroke="url(#paint0_linear_179_495)" />
                                    <circle cx="4.5" cy="105.5" r="4.5" fill="white" />
                                    <circle cx="4.5" cy="13.5" r="4.5" fill="#FFB017" />
                                    <circle cx="4.5" cy="59.5" r="4.5" fill="white" />
                                    <defs>
                                        <linearGradient id="paint0_linear_179_495" x1="4" y1="123" x2="4" y2="0"
          ...
          <div class="flex mt-[10px] lg:mt-0 flex-col justify-center lg:pr-[80px] relative">
            <div
              ...
              <div class="flex justify-between lg:mb-[35px] mb-[20px] lg:justify-start">
                ...
                <p class="text-[15px] lg:text-[18px] text-white font-medium leading-[18px] ml-auto">{{ rewardEarnings }}
                  EMC</p>
              </div>
              <div class="flex justify-between lg:mb-[35px] mb-[20px] lg:justify-start">
            ...
            <div class="w-[calc(100%-30px)] lg:w-[calc(100%-80px)] ml-[15px] lg:ml-auto lg:mt-[60px] mt-[40px]">
              ...
              <div class="mt-[15px] flex justify-between items-center w-full h-[54px] rounded-[8px] bg-[#2A2C47]">
                <p class="ml-[15px] text-[24px] leading-[24px] text-white/70 font-medium">
                  <n-input-number :disabled="useETHUser.isInvalidConnect" class="number-input" size="large"
                    :bordered="false" v-model:value="EMC" :update-value-on-input="false" placeholder="" :min="0"
                    :max="balance ? balance : 0" :show-button="false" />
                </p>
              ...
              <div class="flex mt-[20px] w-full justify-between">
                ...
                <p class="text-white/70 text-[16px] leading-[16px] font-medium">{{ typeof locked === 'undefined' ? '---' :
                  locked }} EMC</p>
              </div>
              <div class="flex mt-[20px] w-full justify-between">
                <p class="text-white/70 text-[16px] leading-[16px] font-medium">Available:</p>
                <p class="text-white/70 text-[16px] leading-[16px] font-medium">
                  {{ typeof releasable === 'undefined' || typeof rewards === 'undefined' ? '---' : releasable + rewards }}
                  EMC
                </p>
              </div>
            </div>

            <div v-if="!useETHUser.isInvalidConnect"
              class="flex flex-col justify-center w-[420px] mt-8 mx-auto text-center">
              <div class="flex items-center mb-3 cursor-pointer" @click="onPressEnterAddress">
                ...
                <div
                  class="w-full h-8 leading-8 px-4 ml-2 rounded-[4px] text-center bg-[#323557] text-white/88 text-[14px] font-normal overflow-hidden">
                  {{ transferAddress }}
                </div>
              </div>
              <p class="text-[#B6B5BE] text-[12px] leading-[16px] font-normal">
                After the staking ends, only the address owner can withdraw all EMC and other earnings. Please ensure all
    ...
    <n-modal v-model:show="decompressionModal">
      <n-card class="withdraw-modal w-[calc(100%-30px)] md:w-[550px]" title="" :bordered="false" size="huge" role="dialog"
        aria-modal="true">
        <div class="flex flex-col items-center relative bg-[#1A1C34] rounded-[24px]">
          ...
          <div class="px-[15px] md:px-[66px] mt-[20px] md:mt-[40px] w-full">
            ...
            <div class="flex mt-[20px] w-full justify-between">
              <p class="text-white/70 text-[16px] leading-[16px] font-medium">Balance:</p>
              <p class="text-white/70 text-[16px] leading-[16px] font-medium">{{ typeof balance === 'undefined' ? '---' :
                balance }} EMC</p>
            </div>
            <div class="flex mt-[20px] w-full justify-between">
              <p class="text-white/70 text-[16px] leading-[16px] font-medium">Staked:</p>
...
// erc20 balcnce
const erc20Balance = ref<number | undefined>(undefined);
// 打入的总奖励余额
const permanentTotal = ref<number | undefined>(undefined);
//earnings
const earnings = ref<number | undefined>(undefined);
// 可释放的emc
...
const staking = async () => {
  ...
  const formData: FormData = {
    start: parseInt(String(timestamp.value / 1000)),
    cycles: phase.value,
...
const tradingListInit = async () => {
  ...
  const resp = await http.get({
    ...
    data: {
      ...
      extra: encodeURIComponent(JSON.stringify({ beneficiary: account0.value.toLocaleLowerCase() })),
    },
  });

  tradingList.value = (resp.data || []).map((item: any) => {
    ...
    const amount = getAmount(Number(item.amountTotal), decimals.value!);
...
watch(
  ...
  () => {
    ...
    rewardEarnings.value = calcEarning(EMC.value, currentDay.value.nAPR, currentDay.value.day);
  },
  { immediate: true }
);
...
onMounted(async () => {
  ...
  await erc20Api!
    .balanceOf({ account: emcContract })
    .then((res) => {
      erc20Balance.value = getAmount(Number(res.data), decimals.value!);
    })
    .catch((err) => {
      console.error(err);
    });
  ...
  earnings.value = getAmount(Number(resp.data?.earnings), decimals.value!);
});
