...
onMounted(async () => {
  loading.value = true;
  const [resp, { fdv, priceUsd }] = await Promise.all([http.get({ url: '/stats/total-supply-all' }), getDexData('solana')]);
  loading.value = false;
  const data = (resp.data || {}) as { [k: string]: { raw: string; num: number } };
  const { value: _value, text, unit: _unit } = formatMillion(data.sol.num * priceUsd);
  value.value = `$${text}`;
